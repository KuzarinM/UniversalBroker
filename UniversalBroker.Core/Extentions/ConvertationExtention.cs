...
namespace UniversalBroker.Core.Extentions
{
    public static class ConvertationExtention
    {
        ...
        public static T GetModelFromAttributes<T>(this Dictionary<string, string> attributes)
        {
        ...
        public static int SetValueFromAttributes<T>(this T model, Dictionary<string, string> attributes)
        {
            var type = typeof(T);
            ...
            foreach (var property in type.GetProperties())
            {
                try
                {
                    if (property.CanWrite)
                    {
                        if(attributes.TryGetValue($"{type.Name}.{property.Name}", out var val) || attributes.TryGetValue(property.Name, out val))
                        {
                            TypeConverter converter = TypeDescriptor.GetConverter(property.PropertyType);
                            property.SetValue(model, converter.ConvertFromString(val));
            ...
            foreach (var field in type.GetFields())
            {
                try
                {
                    if (field.CanBeSet())
                    {
                        if (attributes.TryGetValue($"{type.Name}.{field.Name}", out var val) || attributes.TryGetValue(field.Name, out val))
                        {
                            TypeConverter converter = TypeDescriptor.GetConverter(field.FieldType);
                            field.SetValue(model, converter.ConvertFromString(val));
        ...
        public static int GetAttributesFromModel<T>(this T model, Dictionary<string,string?> existing)
        {
            var defaultModel = (T)Activator.CreateInstance(typeof(T))!;
        ...
        public static int AddOrUpdateAttribute(this Dictionary<string, string?> attributes, string name, string value)
        {
